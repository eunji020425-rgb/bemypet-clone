  ...
  return (
    <div className="max-w-2xl mx-auto px-4 py-8">
      <div className="flex items-center justify-between mb-6">
        <h1 className="text-xl font-bold text-[#2a3a55]">자유게시판</h1>
        {user && (
          <Link
            href="/community/write"
            className="flex items-center gap-1 bg-[#3a7ab8] hover:bg-[#1a2a3f] text-white text-sm font-bold px-4 py-2 rounded-full transition-colors"
          >
            <PenSquare size={14} />
            글쓰기
          </Link>
        ...
        <div className="flex flex-col gap-3">
          {(posts as (Post & { profiles: { nickname: string; avatar_url: string | null }; comments: { count: number }[] })[]).map(post => (
            <Link
              ...
              className="bg-white rounded-2xl p-5 shadow-sm border border-[#d6e6ff] hover:border-[#3a7ab8] transition-colors"
            >
              <div className="flex items-start justify-between gap-2 mb-2">
                <h2 className="font-medium text-[#2a3a55] text-sm line-clamp-2 flex-1">{post.title}</h2>
                {post.image_url && (
