...
export default function ChatRoom({ initialMessages, userId, userNickname }: Props) {
  ...
  return (
    <div className="bg-white rounded-2xl shadow-sm border border-[#d6e6ff] flex flex-col" style={{ height: '70vh' }}>
      {/* 메시지 목록 */}
        ...
        {messages.map((msg, idx) => {
          ...
          return (
            <div key={msg.id} className={`flex gap-2 ${isMe ? 'flex-row-reverse' : 'flex-row'}`}>
              {/* 아바타 (내 메시지는 숨김) */}
              {!isMe && (
                <div className={`w-7 h-7 rounded-full bg-[#b8d3f5] flex items-center justify-center text-xs font-bold text-[#3a7ab8] flex-shrink-0 ${showAvatar ? 'opacity-100' : 'opacity-0'}`}>
      ...
      <div className="border-t border-[#d6e6ff] px-4 py-3">
        <form onSubmit={handleSend} className="flex gap-2">
          <input
            ...
            disabled={!userId}
            className="flex-1 border border-[#d6e6ff] rounded-full px-4 py-2 text-sm outline-none focus:border-[#3a7ab8] disabled:bg-gray-50"
            maxLength={500}
          />
          <button
            type="submit"
        ...
        {!userId && (
          <p className="text-xs text-center text-[#aaa] mt-2">
            <a href="/auth/login" className="text-[#3a7ab8] hover:underline">로그인</a>하고 채팅에 참여하세요
          </p>
        )}
      </div>
    </div>
